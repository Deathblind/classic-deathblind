...
import {Featured, FeaturedProps} from "../../../ui/components/post/featured/featured";
...
const mapStateToFeaturedProps = (state: RootState, ownProps: ConnectedToPostProps): FeaturedProps => {
    const post = getPostById(ownProps.postId)(state);
    const author = post && getAuthorById(post.author)(state);

    return {
        image: "http://www.deathblind.com/wp-content/uploads/2017/06/WorldOfWarcraft_art36_1.jpg",
        imageAlt: "",
...
export const ConnectedFeatured = connect(
    mapStateToFeaturedProps
)(Featured);

export const StyledLatestPostHightlight = styled(ConnectedFeatured)`
    max-width: ${({theme}) => `${theme.gridSpan * 128}em`};
    width: 100%;
`;

const mapStateToTileProps = (state: RootState, ownProps: ConnectedToPostProps): TileProps => {
    const post = getPostById(ownProps.postId)(state);
    const author = post && getAuthorById(post.author)(state);

    return {
        image: "http://www.deathblind.com/wp-content/uploads/2018/11/Comp-1_00000.png",
        title: post ? post.title.rendered : "",
        author: author ? author.name : "",
        tags: [],
        category: ""
...
export const LatestPosts: SFC<LatestPostsProps> = memo(({ posts }) => {
    ...
    return <StyledLatestPosts>
        <h2>Latest Posts</h2>

        {latestPost && <StyledLatestPostHightlight
            postId={latestPost}
        />}

        {others.length && <StyledLatestPostGrid>
            {others.map(post => <ConnectedTile
                key={post}
                postId={post}
            />)}
